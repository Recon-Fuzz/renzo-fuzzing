...
abstract contract RestakeManagerTargetsV2 is BaseTargetFunctions, SetupV2 {
    ...
    function restakeManager_slash_native(uint256 operatorDelegatorIndex) public {
        ...
        try ethPOSDepositMock.slash(1 ether) {
            // emit Debug(address(ethPOSDepositMock).balance);
            // t(false, "call to ethPOSDepositMock.slash succeeds");
        } catch {
    ...
    // function restakeManager_test_slash_native(uint256 _operatorDelegatorIndex) public payable {
    ...
    //     // @audit this was being skipped in initial test but is likely the source of reverts causing issues
